...
def stair(length, width, height, steps):
	x = length / float(steps)
	z = height / float(steps)
	stair_a = STRUCT( NN(steps)( [ CUBOID([ x, width, z ]), T([1,3])([ x, z ]) ] ) )
	triangle = MKPOL([ [ [ 0, 0 ], [ 0, z ], [ x, z  ]  ], [ [ 1, 2, 3] ], None ])
	prism = T([1])([x])( MAP([S1,S3,S2])( PROD([ triangle, Q(width) ]) ) )
	stair_b = STRUCT( NN(steps - 1)( [ prism, T([1,3])([ x, z ]) ] ) )
	return STRUCT([ stair_a, stair_b ])

# stair1

stair1 = T([1,2])([ square_side + 31, square_side + distance_y + square_side ])(stair(50, stairs_width, distance_between_floors + floor_thickness, 80 / 4.0))

# stair2

stair2 = T([1,2,3])([ square_side + 15, square_side + distance_y + square_side, distance_between_floors + floor_thickness ])(stair(50, stairs_width, distance_between_floors + floor_thickness, 80 / 4.0))

# stair3

stair3 = T([1,2,3])([ square_side + 15 + 50 + 11, square_side + distance_y + square_side, 2 * (distance_between_floors + floor_thickness) ])(stair(50, stairs_width, distance_between_floors + floor_thickness, 80 / 4.0))

# stairs_cage

def ring_slice(r1, r2, alpha):
	def aux(m):
		x = QUOTE(N(m)(alpha / float(m)))
		y = T([1])([r1])(QUOTE([r2-r1]))
		domain = PROD([ x, y ])
		def mapping(p):
...
r1 = stairs_width + square_side / 2.0

rs = ring_slice(r1, r1 + square_side, PI)(32)

rs_3D = PROD([ rs, Q(distance_between_floors) ])

stairs_cage_a = T([1,2])([r1, -12])(CUBOID([ square_side, 12, distance_between_floors ]))

stairs_cage = T([1,2])([ 12 + 3 * (square_side + distance_x) + square_side / 2.0, square_side + distance_y + square_side / 2.0 ])( R([1,2])(-PI/2)(STRUCT([ stairs_cage_a, rs_3D ])) )

# stairs

stairs = STRUCT([ stair1, stair2, stair3, stairs_cage ])

# building (pillars, horizontal partitions, vertical enclosures, windows and stairs)
