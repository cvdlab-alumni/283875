...
def glass1():
	pp1 = [
		[ -54, 15, 61 ],
		[ (-64 -15) / 2.0 - 3, (15 + 40) / 2.0 + 3, (61 + 46) / 2.0 ],
		[ -15, 40, 46 ]
	]
	pp2 = [
		[ -64, 18, 61 / 2.0 ],
		[ (-64 -15) / 2.0 - 5, (18 + 43) / 2.0 + 5, (61 + 46) / 4.0 ],
		[ -15, 43, 46 / 2.0 ]
	]
	pp3 = [
		[ -64, 18, 0 ],
		[ (-64 -15) / 2.0 - 2, (18 + 45) / 2.0 + 2, 0 ],
		[ -15, 45, 0 ]
	]
	c1 = BEZIER(S1)(pp1)
	c2 = BEZIER(S1)(pp2)
	...
	ss = STRUCT([ ss1, ss2 ])
	# Alphas not working!
	return COLOR([ 0.8, 0.9, 1 ])(ss)

def glass2():
	pp1 = [
		[ 89, 40, 43 ],
		[ (89 + 145) / 2.0 + 5, (40 + 21) / 2.0 + 5, 43 ],
		[ 145, 21, 39 ]
	]
	pp2 = [
		[ 89, 40, 43 / 2.0 ],
		[ (89 + 150) / 2.0 + 5, (40 + 21) / 2.0 + 5, (43 + 39) / 4.0 ],
		[ 150, 21, 39 / 2.0 ]
	]
	pp3 = [
		[ 89, 40, 0 ],
		[ (89 + 150) / 2.0 + 2, (40 + 21) / 2.0 + 2, 0 ],
		[ 150, 21, 0 ]
	]
	c1 = BEZIER(S1)(pp1)
	c2 = BEZIER(S1)(pp2)
	...
	ss = STRUCT([ ss1, ss2 ])
	# Alphas not working!
	return COLOR([ 0, 0, 0 ])(ss)

#################### Exercise 5 ####################

glasses = STRUCT([ glass1(), glass2() ])

#################### Output ####################

VIEW(STRUCT([ profiles, wheels, steering_wheel, glasses ]))
