...
// Unique LAR representation from several LAR representations
function lar( models ) {
	var vertices = [];
	var faces = [];
	var num = 0;
	...
	for ( i = 0; i < models.length; i++ ) {
		// Add vertices
		for ( j = 0; j < models[i].vertices.length; j++ ) {
			vertices.push( models[i].vertices[j] );
		};
		// Add faces (note vertices' indexes calculation)
		for ( j = 0; j < models[i].faces.length; j++ ) {
			faces.push( models[i].faces[j].map( function (x) {
				return num + x;
			} ) );
		};
		// Update vertices count
		num = num + models[i].vertices.length;
	};
...
var models = CAT( [ [ t, l, s ], tt.trees, b.buildings ] );

console.log( obj( lar( models ) ) );